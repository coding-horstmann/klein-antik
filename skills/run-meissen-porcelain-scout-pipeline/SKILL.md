...
6. Collect or reuse a fresh frozen batch from approved non-eBay sources. Keep `Meissen` as an explicit discovery query and add bounded broad porcelain-category discovery where the collector supports it. For Blocket, DBA and Tori, the governed Railway importer must collect the offer before `scripts/export_marketplace_deals.py` exports its frozen batch; never query those marketplaces directly from a browser. A broad-category result needs later image or mark evidence before it may be considered Meissen. For visual diagnosis, use only the Codex in-app Browser and never use it for bulk collection.
